...
filename = string(ident, ".h5")
dvp_home = "/idiap/user/kmatoba/data-visualization-project"
base_dir = string(dvp_home, "/acas")
gentable_dir = string(base_dir, "/GenerateTable")
qtable_dir = string(gentable_dir, "/Qtables")

if !isdir(gentable_dir)
    mkdir(gentable_dir)
end
...
saveFile = string(qtable_dir, "/", filename)
...
function compute_trans_reward(mdps::Array{Union{MDP, POMDP},1},
...
println(string("Writing Qvalues to ", saveFile))
